...
async function renderRegister(){
 ...
 all.forEach(a=>{const e=evidenceFor(a),pre=window.HPOS_HALAL_AUTOSCREEN?.cached(a),k=e?.state||(pre?.state==='FAIL'?'FAIL':'UNKNOWN');counts[k]=(counts[k]||0)+1});
 const rows=all.map(a=>{const e=evidenceFor(a),pre=window.HPOS_HALAL_AUTOSCREEN?.cached(a),state=e?.state||(pre?.state==='FAIL'?'FAIL':'UNKNOWN'),status=e?uiState(state):(pre?.state==='FAIL'?'NICHT HALALKONFORM':'PRÜFUNG OFFEN'),sub=e?'Evidenz geprüft':pre?(pre.state==='FAIL'?'AAOIFI: Ausschluss erkannt':'AAOIFI: Prüfung offen'):'Automatik: wartet';return '<button class="row halalRegisterRow" data-halal-asset="'+encodeURIComponent(String(a.isin||a.ticker||a.name))+'"><span class="avatar">'+String(a.name||'?').split(/\s+/).filter(Boolean).slice(0,2).map(x=>x[0]).join('').toUpperCase()+'</span><span><strong>'+escapeHtml(a.name||a.ticker||a.isin)+'</strong><small>'+escapeHtml(a.scope)+' · '+escapeHtml(sub)+'</small></span><span class="right '+cls(state)+'"><strong>'+status+'</strong></span></button>'}).join('');
 box.innerHTML='<div class="cards"><div class="card"><small>Halalkonform</small><strong class="pos">'+counts.PASS+'</strong></div><div class="card"><small>Prüfung offen</small><strong class="warn">'+(counts.OPEN_REVIEW+counts.UNKNOWN)+'</strong></div></div><div class="panel spaced"><div class="labelWithInfo"><strong>Automatische Prüfliste</strong><button type="button" class="infoBtn" data-info-eye="Halal Register" data-info-title="Automatische Halal-Prüfung" data-info-html="Depot- und Watchlist-Werte werden automatisch aufgenommen. Reihenfolge: freigegebene ISIN-Evidenz, HPOS AAOIFI Rule Engine, kostenlose externe Gegenprüfung und als letzte Instanz manuell eingefügte externe Evidenz. Kostenpflichtige Tarife sind blockiert. Fehlende Daten bleiben PRÜFUNG OFFEN." aria-label="Automatische Prüfung erklären">i</button></div><p class="sub">Halal Terminal Free: <strong class="'+(providerStatus.configured&&providerStatus.freeOnlyAllowed!==false?'pos':'warn')+'">'+(providerStatus.configured?(providerStatus.freeOnlyAllowed===false?'BLOCKIERT · nicht Free':'bereit · Free'):'noch nicht verbunden')+'</strong></p><button id="halalRefreshAll" class="secondary full">Halal-Prüfung aktualisieren</button></div><div class="section"><div class="sectionHead"><h2>Prüfstatus</h2><span class="eye">'+all.length+' Werte</span></div><div class="list">'+(rows||'<div class="empty">Keine Werte vorhanden.</div>')+'</div></div>';
