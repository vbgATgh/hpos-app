...
function addVerified(a){if(!validIsin(a.isin)||inDepot(a))return false;const w=readWatch(),k=up(a.isin);if(!w.some(x=>up(x.isin)===k))w.push(canonical(a,a.source||'VERIFIED_SEARCH'));writeWatch(w);return true}
function esc(v){return String(v||'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
function resultRow(a){if(inDepot(a))return `<div class="guardResult"><div><strong>${esc(a.name)}</strong><small>${esc([a.isin,a.ticker].filter(Boolean).join(' · '))} · ISIN verifiziert</small></div><span class="guardDepot">Im Depot</span></div>`;const id=encodeURIComponent(JSON.stringify(a));return `<div class="guardResult"><div><strong>${esc(a.name)}</strong><small>${esc([a.isin,a.ticker].filter(Boolean).join(' · '))} · ISIN verifiziert</small></div><button class="guardAdd" data-security="${id}">+ Watchlist</button></div>`}
...
function wire(){migrateWatch();const q=$('#securityQuery');if(q){let t;q.addEventListener('input',()=>{clearTimeout(t);t=setTimeout(()=>search(q.value),320)});q.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();clearTimeout(t);search(q.value)}})}}
