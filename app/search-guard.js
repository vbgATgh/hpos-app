...
function renderExternal(list,label='Globale Treffer'){let box=$('#verifiedResults');if(!box){box=document.createElement('div');box.id='verifiedResults';box.className='verifiedResults';$('#searchResults')?.insertAdjacentElement('afterend',box)}box.innerHTML=list.length?`<div class="eye guardEye">${esc(label)}</div>${list.map(resultRow).join('')}`:'';box.querySelectorAll('.guardAdd').forEach(b=>b.onclick=()=>{const a=JSON.parse(decodeURIComponent(b.dataset.security));if(addWatch(a)){b.textContent='✓ Watchlist';try{sessionStorage.setItem(RESTORE_KEY,JSON.stringify({q:$('#securityQuery')?.value||a.isin||a.ticker,at:Date.now()}))}catch{}setTimeout(()=>location.reload(),250)}else if(inDepot(a)){b.textContent='Im Depot'}else if(inWatch(a)){b.textContent='✓ Watchlist'}})}
