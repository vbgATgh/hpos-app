...
function validTicker(v){v=up(v);return /^[A-Z0-9.^=\-]{1,16}$/.test(v)&&!/^\d+$/.test(v)}
...
function inWatch(a){const isin=up(a?.isin);return !!isin&&readWatch().some(x=>up(x?.isin)===isin)}
...
function inDepot(a){const isin=up(a?.isin);return !!isin&&readDepotIsins().has(isin)}
function key(a){return up(a.isin||a.ticker||a.symbol||a.name)}
function canonical(a,source='VERIFIED_SEARCH'){return{name:norm(a.name||a.shortname||a.longname||a.symbol||a.ticker||a.isin),isin:up(a.isin),ticker:up(a.ticker||a.symbol),source,verified:validIsin(a.isin),addedAt:new Date().toISOString()}}
function migrateWatch(){const before=readWatch(),seen=new Set(),after=[];for(const x of before){const isin=up(x.isin),ticker=up(x.ticker);if(!validIsin(isin)||inDepot({isin}))continue;const k=isin||ticker;if(!k||seen.has(k))continue;seen.add(k);after.push({...x,isin,ticker,verified:x.verified===true&&validIsin(isin)})}if(JSON.stringify(before)!==JSON.stringify(after))writeWatch(after)}
...
function merge(...lists){const out=[],seen=new Set();for(const a of lists.flat()){if(!validIsin(a?.isin))continue;const k=up(a.isin);if(!k||seen.has(k))continue;seen.add(k);out.push(a)}return out}
function addVerified(a){if(!validIsin(a.isin)||inDepot(a)||inWatch(a))return false;const w=readWatch();w.push(canonical(a,a.source||'VERIFIED_SEARCH'));writeWatch(w);return true}
function esc(v){return String(v||'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
function resultRow(a){if(inDepot(a))return `<div class="guardResult"><div><strong>${esc(a.name)}</strong><small>${esc([a.isin,a.ticker].filter(Boolean).join(' · '))} · ISIN verifiziert</small></div><span class="guardDepot">Im Depot</span></div>`;if(inWatch(a))return `<div class="guardResult"><div><strong>${esc(a.name)}</strong><small>${esc([a.isin,a.ticker].filter(Boolean).join(' · '))} · ISIN verifiziert</small></div><span class="guardDepot">✓ Watchlist</span></div>`;const id=encodeURIComponent(JSON.stringify(a));return `<div class="guardResult"><div><strong>${esc(a.name)}</strong><small>${esc([a.isin,a.ticker].filter(Boolean).join(' · '))} · ISIN verifiziert</small></div><button class="guardAdd" data-security="${id}">+ Watchlist</button></div>`}
...
function renderExternal(list,label='ISIN-verifizierte Treffer'){let box=$('#verifiedResults');if(!box){box=document.createElement('div');box.id='verifiedResults';box.className='verifiedResults';$('#searchResults')?.insertAdjacentElement('afterend',box)}box.innerHTML=list.length?`<div class="eye guardEye">${esc(label)}</div>${list.map(resultRow).join('')}`:'';box.querySelectorAll('.guardAdd').forEach(b=>b.onclick=()=>{const a=JSON.parse(decodeURIComponent(b.dataset.security));if(addVerified(a)){b.textContent='✓ Watchlist';try{sessionStorage.setItem(RESTORE_KEY,JSON.stringify({q:$('#securityQuery')?.value||a.isin,at:Date.now()}))}catch{}setTimeout(()=>location.reload(),250)}else if(inDepot(a)){b.textContent='Im Depot'}else if(inWatch(a)){b.textContent='✓ Watchlist'}})}
...
const local=localSearch(q),external=await externalSearch(q),all=merge(external,local);renderExternal(all,all.length?'ISIN-verifizierte Treffer':'');if(!all.length)statusBox('Kein ISIN-verifizierter Treffer gefunden. Für eine eindeutige Identifikation bitte die ISIN verwenden.');return all}
