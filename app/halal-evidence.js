...
async function render(){
 ...
 if(e.state==='OPEN_REVIEW'&&!registry?.assets?.[id.isin]&&window.HPOS_HALAL_MANUAL){manual=window.HPOS_HALAL_MANUAL.record();if(manual?.identityConfirmed&&manual?.standard==='AAOIFI'){if(manual.state==='PASS')e={state:'PASS',reason:'Manuell bestätigte externe AAOIFI-Evidenz von '+manual.provider+' wurde der verifizierten ISIN '+id.isin+' zugeordnet.',source:manual.provider+' · Copy-Paste',reviewedAt:manual.createdAt,evidence:[{provider:manual.provider,status:'AAOIFI_PASS_MANUAL',note:'Vom Nutzer bestätigte externe Evidenz'}]};else if(manual.state==='FAIL')e={state:'FAIL',reason:'Manuell bestätigte externe AAOIFI-Evidenz von '+manual.provider+' meldet FAIL.',source:manual.provider+' · Copy-Paste',reviewedAt:manual.createdAt,evidence:[{provider:manual.provider,status:'AAOIFI_FAIL_MANUAL',note:'Vom Nutzer bestätigte externe Evidenz'}]};}}
 if(e.state==='OPEN_REVIEW'&&!registry?.assets?.[id.isin]&&id.isin&&window.HPOS_HALAL_PROVIDER){
   ...
   if(provider?.verdict==='COMPLIANT')e={state:'PASS',reason:'Halal Terminal meldet dieses Instrument als Shariah-compliant. HPOS ordnet das Ergebnis der verifizierten ISIN '+id.isin+' und dem verwendeten Marktsymbol '+id.ticker+' zu.',source:'Halal Terminal Free',reviewedAt:provider.checkedAt,evidence:[{provider:'Halal Terminal',status:'COMPLIANT',note:'Free-Tier Screening · '+(provider.rawStatus||provider.verdict)}]};
   else if(provider?.verdict==='NON_COMPLIANT')e={state:'FAIL',reason:'Halal Terminal meldet dieses Instrument als nicht Shariah-compliant. HPOS blockiert die nachfolgenden Gates.',source:'Halal Terminal Free',reviewedAt:provider.checkedAt,evidence:[{provider:'Halal Terminal',status:'NON_COMPLIANT',note:'Free-Tier Screening · '+(provider.rawStatus||provider.verdict)}]};
 }
