...
async function render(){
 ...
 if(e.state==='OPEN_REVIEW'&&!registry?.assets?.[id.isin]&&window.HPOS_HALAL_MANUAL){
   ...
   if(manual?.identityConfirmed&&manual?.standard==='AAOIFI'){
     if(manual.state==='PASS')e={state:'PASS',reason:'Manuell bestätigte externe AAOIFI-Evidenz von '+manual.provider+' wurde der verifizierten ISIN '+id.isin+' zugeordnet.',source:manual.provider+' · Copy-Paste',reviewedAt:manual.createdAt,evidence:[{provider:manual.provider,status:'AAOIFI_PASS_MANUAL',note:'Vom Nutzer bestätigte externe Evidenz'}]};
     else if(manual.state==='FAIL')e={state:'FAIL',reason:'Manuell bestätigte externe AAOIFI-Evidenz von '+manual.provider+' meldet FAIL.',source:manual.provider+' · Copy-Paste',reviewedAt:manual.createdAt,evidence:[{provider:manual.provider,status:'AAOIFI_FAIL_MANUAL',note:'Vom Nutzer bestätigte externe Evidenz'}]};
   }
