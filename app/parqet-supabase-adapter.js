(()=>{'use strict';
const API='https://moxyhjfbrmsnphikxqje.supabase.co/functions/v1/hpos-api';
...
function captureOAuthSession(){const hash=new URLSearchParams(location.hash.replace(/^#/,''));if(hash.get('parqet')!=='connected')return;const s=String(hash.get('session')||''),e=String(hash.get('sessionExpires')||'');if(!/^[A-Za-z0-9_-]{20,}$/.test(s))return;localStorage.setItem(SESSION_KEY,s);if(e)localStorage.setItem(SESSION_EXP_KEY,e);localStorage.setItem(COMPAT_KEY,'SUPABASE_ADAPTER');history.replaceState(null,'',location.pathname+location.search)}
...
function reconnect(reason='reauth'){localStorage.setItem(REAUTH_GUARD,reason);location.href=API+'/auth/parqet/start'}
function parsed(input){try{return new URL(typeof input==='string'?input:input.url,location.href)}catch{return null}}
function storedLive(){try{const d=JSON.parse(localStorage.getItem(VALID_KEY)||'null');return d&&Array.isArray(d.holdings)&&d.holdings.length>=10?d:null}catch{return null}}
...
function compatPortfolio(saved){return{holdings:(saved?.holdings||[]).map(h=>{const avg=Number(h?.averagePrice??h?.avgPrice??h?.avg??0);return{...h,avg,avgPrice:avg,averagePrice:avg}}),cash:Number(saved?.cash||0),dividends:saved?.dividends||[],source:saved?.source,savedAt:saved?.savedAt}}
function installRefreshHook(){/* canonical3+: app.js owns the refresh UI; this adapter owns Parqet transport */}
async function bootstrapLive(){if(!session())return;try{const saved=storedLive(),age=saved?.savedAt?Date.now()-Date.parse(saved.savedAt):Infinity,live=saved?.source==='PARQET_LIVE_SYNC';if(live&&age<5*60*1000)return;applyNormalized(await normalizedPortfolio());location.reload()}catch(e){if(e?.code==='REAUTH')return;console.warn('HPOS live bootstrap',String(e?.message||e))}}

captureOAuthSession();if(!localStorage.getItem(COMPAT_KEY))localStorage.setItem(COMPAT_KEY,'SUPABASE_ADAPTER');
window.fetch=async function(input,init){const u=parsed(input);if(u&&u.hostname===LEGACY_HOST&&u.searchParams.get('s')==='parqet'){try{const data=await normalizedPortfolio();return new Response(JSON.stringify({holdings:data.holdings.map(appHolding),cash:Number(data?.cash||0),dividends:[],source:'PARQET_LIVE_SYNC'}),{status:200,headers:{'Content-Type':'application/json','Cache-Control':'no-store'}})}catch(e){const status=e?.code==='REAUTH'?401:502;return new Response(JSON.stringify({error:e?.code==='REAUTH'?'parqet_reauth_required':'parqet_supabase_unavailable',message:String(e?.message||e)}),{status,headers:{'Content-Type':'application/json','Cache-Control':'no-store'}})}}return originalFetch(input,init)};
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{installRefreshHook();bootstrapLive()},{once:true});else{installRefreshHook();bootstrapLive()}
})();
