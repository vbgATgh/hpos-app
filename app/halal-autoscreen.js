...
const KEY='hpos_halal_prescreen_v7',TTL=7*24*60*60*1000;
let curatedFinancials=null,curatedLoading=null;
const RULES=Object.freeze({impureIncomeMax:0.05,interestAssetsMax:0.30,interestDebtMax:0.30,autoPassSafetyMax:0.27,standard:'AAOIFI SS21',marketValueBasis:'MARKET_CAP_AT_CHECK'});
const HARD=[
...
function mergeCurated(p,c){if(!c)return p;const out={...(p||{}),dataQuality:{...(p?.dataQuality||{})},metricSources:{...(p?.metricSources||{})}},metrics=c.metrics||{},business=c.businessProfile;if(business?.description&&business?.sourceUrl&&business?.period){out.businessSummary=business.description;out.profileSource='OFFICIAL_REPORT_CURATED';out.businessSource={sourceType:'OFFICIAL_REPORT_CURATED',sourceName:business.sourceName||'',sourceUrl:business.sourceUrl,period:business.period}}for(const key of ['revenue','totalDebt','interestBearingAssetsUpperBound','interestIncome']){const e=metrics[key],period=e?.period||c.period||'';if(!e||typeof e.value!=='number'||!Number.isFinite(e.value)||!e.sourceUrl||!period)continue;out[key]=e.value;out.dataQuality[key==='totalDebt'?'debt':key==='interestBearingAssetsUpperBound'?'interestAssets':key]=true;out.metricSources[key]={sourceType:'OFFICIAL_REPORT_CURATED',sourceName:e.sourceName||c.sourceName||'',sourceUrl:e.sourceUrl,supportingSources:Array.isArray(e.supportingSources)?e.supportingSources:[],period,page:e.page||null,label:e.label||''}}out.evidenceAsOf=c.checkedAt||c.periodEnd||'';out.fundamentalSource='OFFICIAL_REPORT_CURATED';return out}
...
function financial(p){
 const mv=Number(p?.marketValueAtCheck)||Number(p?.marketCap)||0,revenue=Number(p?.revenue)||0,debt=Number(p?.totalDebt)||0,interestAssets=Number(p?.interestBearingAssetsUpperBound)||0,interestIncome=Math.abs(Number(p?.interestIncome)||0),q=p?.dataQuality||{};
 ...
 return{
   marketValueAtCheck:mv||null,marketValueAsOf:p?.marketValueAsOf||p?.fetchedAt||'',marketValueMethod:p?.marketValueMethod||'UNAVAILABLE',
...
function derive(p){
 const b=classifyBusiness(p),f=financial(p),mvOk=f.marketValueAtCheck>0;
 const criteria={
   business:{rule:'Zulässiges Kerngeschäft',limit:null,state:b.state==='FAIL'?'FAIL':b.state==='PASS_PARTIAL'?'PASS':'OPEN',value:b.category||null,source:p?.businessSource?.sourceType||p?.profileSource||p?.source||'FREE_PROFILE'},
   ...
   interestAssets:{rule:'Zinstragende Vermögenswerte / Marktwert am Prüftag',limit:RULES.interestAssetsMax,state:!mvOk||f.interestAssetsRatio==null?'OPEN':f.interestAssetsRatio<=RULES.interestAssetsMax?'PASS':'OPEN',value:f.interestAssetsRatio,source:f.interestAssetsRatio==null?'FEHLT':sourceFor(f,'interestBearingAssetsUpperBound','marketValueAtCheck')},
   interestDebt:{rule:'Zinstragende Schulden / Marktwert am Prüftag',limit:RULES.interestDebtMax,state:!mvOk||f.debtRatio==null?'OPEN':f.debtRatio<=RULES.interestDebtMax?'PASS':'OPEN',value:f.debtRatio,source:f.debtRatio==null?'FEHLT':sourceFor(f,'totalDebt','marketValueAtCheck')}
 };
