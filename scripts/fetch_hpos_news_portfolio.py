...
und der mobile Feed performant bleiben. Offizielle Primärdomains der realen
Depotwerte werden aus config/asset_sources.json übernommen.
"""
...
def source_domains():
    try:
        ...
        return {}
    return {str(a.get('isin') or '').upper():[a['domain']] for a in data.get('assets',[]) if a.get('isin') and a.get('domain')}

def expand(config,snapshot,now=None):
    now=now or dt.datetime.now(dt.timezone.utc)
    sync=config.get('parqetSync') or {}
    if not sync.get('enabled',True): return config,{'portfolio':0,'watchlistTotal':0,'watchlistFetched':0,'primaryMapped':0}
    assets=[dict(a) for a in config.get('assets',[])]
    by_isin={str(a.get('isin') or '').upper():a for a in assets if a.get('isin')}
    domains=source_domains();portfolio_added=0;primary_mapped=0
    for h in snapshot.get('holdings',[]):
        isin=str(h.get('isin') or '').upper()
        if not isin: continue
        primary=domains.get(isin,[])
        if primary: primary_mapped+=1
        if isin in by_isin:
            a=by_isin[isin]
            # Reale Position schlägt eine frühere Watchlist-Klassifizierung.
            a['scope']='PORTFOLIO';a['enabled']=True
            if primary:a['primaryDomains']=primary
            continue
        a={'assetKey':key(isin),'name':h.get('name') or isin,'isin':isin,
           'aliases':[h.get('name') or isin,isin],'enabled':True,'scope':'PORTFOLIO',
           'queries':[quoted(h.get('name') or isin)],'primaryDomains':primary,'generatedFrom':'PARQET_HOLDING'}
    ...
    batch=max(1,int(sync.get('watchlistBatchSize',15)));hours=max(1,int(sync.get('watchlistRotationHours',12)))
    if dynamic_watch:
        slot=int(now.timestamp()//(hours*3600));start=(slot*batch)%len(dynamic_watch)
    ...
    meta={'portfolio':sum(1 for a in assets if a.get('scope')=='PORTFOLIO' and a.get('enabled')),
          ...
          'portfolioAdded':portfolio_added,'primaryMapped':primary_mapped,'rotationHours':hours,'batchSize':batch}
    return config,meta

def main():
    config=load(BASE);snapshot=load(SNAP)
    runtime,meta=expand(config,snapshot);dump(RUNTIME,runtime)
    spec=importlib.util.spec_from_file_location('hpos_news_base',MODULE)
    mod=importlib.util.module_from_spec(spec);sys.modules[spec.name]=mod;spec.loader.exec_module(mod);mod.CONFIG_PATH=RUNTIME
    try: rc=mod.main()
    finally:
        try:RUNTIME.unlink()
        except FileNotFoundError:pass
    try:
        st=load(STATUS);st['portfolioAssetsConfigured']=meta['portfolio'];st['portfolioPrimarySourcesMapped']=meta['primaryMapped'];st['watchlistAssetsConfigured']=meta['watchlistTotal'];st['watchlistAssetsFetchedThisRun']=meta['watchlistFetched'];st['watchlistRotationHours']=meta['rotationHours'];st['message']=f"{st.get('message','')} Depot: {meta['portfolio']} Titel vollständig; offizielle Primärquellen: {meta['primaryMapped']}/{meta['portfolio']}; Watchlist: {meta['watchlistFetched']}/{meta['watchlistTotal']} Titel in diesem Rotationslauf."
        dump(STATUS,st)
    except Exception as exc: print(f'[WARN] Status-Erweiterung: {exc}',file=sys.stderr)
    print(f"[SCOPE] Portfolio {meta['portfolio']} · Primärquellen {meta['primaryMapped']}/{meta['portfolio']} · Watchlist {meta['watchlistFetched']}/{meta['watchlistTotal']}")
    return rc
