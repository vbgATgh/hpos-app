...
ASSETS=ROOT/'config'/'asset_sources.json'; THESIS=ROOT/'data'/'thesis_registry.json'; POLICY=ROOT/'config'/'fundamental_source_policy.json'; OUT=ROOT/'data'/'fundamental'/'coverage.json'
...
def match_source(key,t,assets):
    ...
    if target:
        return next((a for a in assets if a.get('isin')==target),None)
    return None
def main():
    assets=json.loads(ASSETS.read_text())['assets']; reg=json.loads(THESIS.read_text())['assets']; pol=json.loads(POLICY.read_text()); rows=[]
