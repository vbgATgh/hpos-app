...
def test_cardinal_case_is_isin_centered_and_loaded():
    ...
    assert 'investment-case.js?v=20260913-mobileux1' in HTML
    assert 'investment-case.css?v=20260913-mobileux1' in HTML
...
def test_case_mounts_even_when_portfolio_fit_arrives_late():
    assert "halal?halal.insertAdjacentElement('beforebegin',s)" in JS
...
def test_mobile_decision_cockpit_has_progressive_disclosure():
    ...
    assert 'data-case-tab="overview"' in JS
    assert 'data-case-tab="analysis"' in JS
    assert 'data-case-tab="evidence"' in JS
    assert "caseFitTrack" in JS
    assert "caseWhy" in JS
    assert "caseValuationScale" in JS
    assert "detail('Depotgewicht')" in JS
    assert "return positionWeight(isin())" in JS
    assert ".caseEnhanced .caseLegacyStatus" in css
    assert ".caseTabs" in css
