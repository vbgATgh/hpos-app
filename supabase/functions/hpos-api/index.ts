...
async function saveHalalEvidence(body:any){
  ...
  if(old?.source_type==="CURATED_ISIN")return halalEvidence(isin);
...
function normalizeDividends(root:any,rawHoldings:any[]){
  ...
  for(const x of activities){
    ...
    const key=dividendKey(isin,date,net,currency);
...
function dividendKey(isin:string,date:string,net:number,currency:string){
  const day=new Date(date).toISOString().slice(0,10);
  return`${isin}|${day}|${currency}|${Math.round(net*100)}`
}
